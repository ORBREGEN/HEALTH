...
DISEASE_COL    = "disease"
DONOR_COL      = "donor_id"
CONTROL_TERMS  = [t.strip().lower() for t in os.environ.get("CONTROL_TERMS", "normal,control,healthy").split(",") if t.strip()]
DISEASE_TERMS  = [t.strip().lower() for t in os.environ.get("DISEASE_TERMS", "covid").split(",") if t.strip()]
